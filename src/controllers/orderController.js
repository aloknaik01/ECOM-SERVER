...
export const confirmCashOnDeliveryOrder = catchAsyncErrors(async (req, res, next) => {
  ...
  await database.query(
    `INSERT INTO payments (order_id, payment_type, payment_status, payment_intent_id)
     ...
     DO UPDATE SET payment_status = 'Pending', payment_type = 'COD', payment_intent_id = EXCLUDED.payment_intent_id`,
    [orderId, `COD-${orderId}`]
  );

  if (!order.paid_at) {
    // Pass false to isPaid so it doesn't set paid_at immediately for COD
    await finalizeOrderAfterPayment(orderId, false);
  }
...
json_build_object(
 ...
 LEFT JOIN order_items oi ON o.id = oi.order_id
 LEFT JOIN shipping_info s ON o.id = s.order_id
WHERE o.buyer_id = $1 AND (o.paid_at IS NOT NULL OR o.order_status != 'Pending')
GROUP BY o.id, s.id
        `,
    [req.user.id]
  );
...
LEFT JOIN order_items oi ON o.id = oi.order_id
LEFT JOIN shipping_info s ON o.id = s.order_id
WHERE o.paid_at IS NOT NULL OR o.order_status != 'Pending'
GROUP BY o.id, s.id
        `);
...
export const updateOrderStatus = catchAsyncErrors(async (req, res, next) => {
  ...
  // If the order is marked Delivered, check if it's a COD order and mark it paid if it isn't already
  if (status === "Delivered") {
    const paymentCheck = await database.query(
      `SELECT payment_type, payment_status FROM payments WHERE order_id = $1`,
      [orderId]
    );

    if (paymentCheck.rows.length > 0) {
      const payment = paymentCheck.rows[0];
      if (payment.payment_type === 'COD' && payment.payment_status === 'Pending') {
        await database.query(
