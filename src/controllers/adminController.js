...
export const dashboardStats = catchAsyncErrors(async (req, res, next) => {
  ...
  const totalRevenueAllTimeQuery = await database.query(`
    SELECT SUM(total_price) FROM orders WHERE paid_at IS NOT NULL OR order_status != 'Pending'
    `);
  ...
  const orderStatusCountsQuery = await database.query(`
      SELECT order_status, COUNT(*) FROM orders WHERE paid_at IS NOT NULL OR order_status != 'Pending' GROUP BY order_status
      `);
  ...
  const todayRevenueQuery = await database.query(
    `
    SELECT SUM(total_price) FROM orders WHERE created_at::date = $1 AND (paid_at IS NOT NULL OR order_status != 'Pending')
    `,
    [todayDate]
  );
  ...
  const monthlySalesQuery = await database.query(`
    ...
    FROM orders WHERE paid_at IS NOT NULL OR order_status != 'Pending'
    GROUP BY month, date
    ORDER BY date ASC
    `);
  ...
  const currentMonthSalesQuery = await database.query(
    `
      ...
      FROM orders 
      WHERE (paid_at IS NOT NULL OR order_status != 'Pending') AND created_at BETWEEN $1 AND $2  
      `,
