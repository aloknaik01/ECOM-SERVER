...
export async function finalizeOrderAfterPayment(orderId) {
  const { rows: existingOrders } = await database.query(
    `SELECT id, paid_at FROM orders WHERE id = $1 LIMIT 1`,
    [orderId]
  );
  ...
  // Idempotency: if this order is already finalized, exit silently.
  if (existingOrders[0].paid_at) {
    return;
  }
